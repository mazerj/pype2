...
/* 4 March '02: EyeLink II checked    */
/* 18 Sept 2002: add current_double_usec() */
/*     For non-commercial use only    */
/*				      */
/*  EyeLink programming interface     */
/*          VERSION 2.5               */

/* UPDATED for EyeLink Windows DLL V2.0 */
/* UPDATED for EyeLink Windows DLL V2.1 */
...
/* understand undocumented EyeLink functions. */
...
/* the data types defined in EYETYPES.H for   */
/* correct sizes.			      */
...
#ifndef CALLTYPE      /* CALLTYPE used for Windows platform or other special calling formats */
 #if defined(_WIN32) || defined(_WIN16)  /* some sort of Windows: using DLL */
  #include <windows.h>
  #define CALLTYPE WINAPI
 #else
  #define CALLTYPE    /* delete if not used */
 #endif
#endif
...
UINT16 CALLTYPE open_eyelink_system(UINT16 bufsize, char FARTYPE *options);

	/* set our EyeLink node name (up to 40 characters) */
void CALLTYPE eyelink_set_name(char FARTYPE *name);

	/* shut down system (MUST do before exiting) */
void CALLTYPE close_eyelink_system(void);
...
UINT32 CALLTYPE current_time(void);       /* current milliseconds (modulo 2^32) */

UINT32 CALLTYPE current_micro(MICRO FARTYPE *m);  /* rtns current milliseconds (modulo 2^32) */

UINT32 CALLTYPE current_usec(void);       /* rtns current microseconds (modulo 2^32) */

void CALLTYPE msec_delay(UINT32 n);       /* delays n msec using current_msec() */
...
INT16 CALLTYPE eyelink_get_node(INT16 resp, void FARTYPE *data);
...
/* NOTE: old packets are discarded if new packet received before reading! */
	...
	/* returns: OK_RESULT or LINK_TERMINATED_RESULT     */
INT16 CALLTYPE eyelink_node_send(ELINKADDR node, void FARTYPE *data, UINT16 dsize);
...
INT16 CALLTYPE eyelink_node_receive(ELINKADDR node, void FARTYPE *data);
...
/********* MESSAGES AND COMMANDS *******/
 ...
 /* and LINK_TERMINATED_RESULT means data could not be sent */
	...
	/* returns: OK_RESULT or LINK_TERMINATED_RESULT */
INT16 CALLTYPE eyelink_send_command(char FARTYPE *text);
...
INT16 CALLTYPE eyelink_command_result(void);

	/* send command string to tracker, wait for reply */
	/* returns: OK_RESULT, NO_REPLY, or error code */
INT16 CALLTYPE eyelink_timed_command(UINT32 msec, char FARTYPE *text);
...
INT16 CALLTYPE eyelink_last_message(char FARTYPE *buf);



	/* send a data file message string to connected tracker */
	/* returns: OK_RESULT or LINK_TERMINATED_RESULT */
INT16 CALLTYPE eyelink_send_message(char FARTYPE *msg);

	/* send a data file message to any or all trackers */
	/* returns: OK_RESULT or LINK_TERMINATED_RESULT    */
INT16 CALLTYPE eyelink_node_send_message(ELINKADDR node, char FARTYPE *msg);



		/* send variable name to tracker for read */
		/* returns: OK_RESULT or LINK_TERMINATED_RESULT */
INT16 CALLTYPE eyelink_read_request(char FARTYPE *text);

		/* checks for reply to eyelink_read_request()     */
		/* returns OK_RESULT if we have it, else NO_REPLY */
		/* copies result to <buf> */
INT16 CALLTYPE eyelink_read_reply(char FARTYPE *buf);


		/* sends request for tracker time update */
		/* returns: OK_RESULT or LINK_TERMINATED_RESULT */
UINT32 CALLTYPE eyelink_request_time(void);
	...
	/* returns: OK_RESULT or LINK_TERMINATED_RESULT      */
UINT32 CALLTYPE eyelink_node_request_time(ELINKADDR node);

	/* checks for reply to eyelink_request_time() */
	/* returns: 0 if no reply, else time */
UINT32 CALLTYPE eyelink_read_time(void);
...
INT16 CALLTYPE eyelink_abort(void);

	/* enters setup mode */
INT16 CALLTYPE eyelink_start_setup(void);

       /* checks if in setup/val/cal/dc mode, or if done */
INT16 CALLTYPE eyelink_in_setup(void);

	/* gets pixel X, Y of target */
	/* returns 1 if visible, 0 if not */
INT16 CALLTYPE eyelink_target_check(INT16 FARTYPE *x, INT16 FARTYPE *y);

	/* call to trigger acceptance of target fixation */
INT16 CALLTYPE eyelink_accept_trigger(void);

	/* start drift correction, specify target position */
INT16 CALLTYPE eyelink_driftcorr_start(INT16 x, INT16 y);

	/* check if drift correction, calibration done */
	/* returns OK_RESULT, ABORT_RESULT, NO_REPLY   */
	/* Reading result resets it to NO_REPLY */
INT16 CALLTYPE eyelink_cal_result(void);

	/* apply correction from last drift correction */
INT16 CALLTYPE eyelink_apply_driftcorr(void);

	/* copies last message from drift correction */
	/* returns message length */
INT16 CALLTYPE eyelink_cal_message(char FARTYPE *msg);
...
INT16 CALLTYPE eyelink_current_mode(void);
...
INT16 CALLTYPE eyelink_position_prescaler(void);

	/* resets data flow at start */
	/* if <clear> deletes all queued data; */
INT16 CALLTYPE eyelink_reset_data(INT16 clear);
  ...
  void FARTYPE * CALLTYPE eyelink_data_status(void);
#else
  void CALLTYPE FARTYPE * eyelink_data_status(void);
#endif

  /* tests for block with samples or events (or both) */
	/* rtns 1 if any of the selected types on, 0 if none on */
INT16 CALLTYPE eyelink_in_data_block(INT16 samples,INT16 events);

	/* waits till a block of samples, events, or both is begun */
	/* maxwait = 0 to just test */
	/* rtns 1 if in block, 0 if not ready */
INT16 CALLTYPE eyelink_wait_for_block_start(UINT32 maxwait,
...
INT16 CALLTYPE eyelink_newest_sample(void FARTYPE *buf);
   ...
   /* FLOATING-POINT DATA TYPES (ALLF_DATA type buffer) */
	...
	/* returns item type: 				     */
	/* SAMPLE_TYPE if sample, 0 if none, else event code */
INT16 CALLTYPE eyelink_get_float_data(void FARTYPE *buf);
...
INT16 CALLTYPE eyelink_data_switch(UINT16 flags);

	/* start data flow */
	/* also aborts any other operations (setup, drift corr.) */
	/* <flags> bits set data output types */
	/* <lock> disables "esc" exit from tracking */
INT16 CALLTYPE eyelink_data_start(UINT16 flags, INT16 lock);

	/* stop data flow */
	/* also aborts any other operations (setup, drift corr.) */
INT16 CALLTYPE eyelink_data_stop(void);
...
INT16 CALLTYPE eyelink_playback_start(void);

	/* stop playback of data file */
INT16 CALLTYPE eyelink_playback_stop(void);
...
INT16 CALLTYPE eyelink_request_image(INT16 type, INT16 xsize, INT16 ysize);

	/* Test image-reception status: */
	/* 0 = not receiving */
	/* -1 = aborted */
	/* 1 = receiving */
INT16 CALLTYPE eyelink_image_status(void);

	/* forces image transmissin to halt */
void CALLTYPE eyelink_abort_image(void);

	/* get data at start of new image */
	/* -1 = aborted/not in rcve */
	/* 0 = old palette, 1 = new palette */
	/* ptrs to size: may be NULL */
INT16 CALLTYPE eyelink_image_data(INT16 FARTYPE *xsize, INT16 FARTYPE *ysize, INT16 FARTYPE *type);

	/* gets unpacked line data */
	/* rtns: -1 if error/not rx */
	/* else rtns line number */
INT16 CALLTYPE eyelink_get_line(void FARTYPE *buf);
...
INT16 CALLTYPE eyelink_get_palette(void FARTYPE *pal);
...
UINT16 CALLTYPE eyelink_read_keybutton(INT16 FARTYPE *mods,
	...
	/* state is KB_PRESS, KB_RELEASE, or KB_REPEAT */
	/* Returns: OK_RESULT or LINK_TERMINATED_RESULT */
INT16 CALLTYPE eyelink_send_keybutton(UINT16 code, UINT16 mods, INT16 state);
...
UINT16 CALLTYPE eyelink_button_states(void);
...
UINT16 CALLTYPE eyelink_last_button_press(UINT32 FARTYPE *time);
...
INT16 CALLTYPE eyelink_flush_keybuttons(INT16 enable_buttons);
...
INT16 CALLTYPE eyelink_request_file_read(char FARTYPE *src);
...
INT16 CALLTYPE eyelink_get_file_block(void FARTYPE *buf, INT32 FARTYPE *offset);

			/* ask for next block of file */
			/* reads from <offset>        */
INT16 CALLTYPE eyelink_request_file_block(UINT32 offset);

	/* aborts send of file */
	/* rtns: 0 if OK, else send error */
INT16 CALLTYPE eyelink_end_file_transfer(void);
...
INT16 CALLTYPE eyelink_bitmap_packet(void *data, UINT16 size, UINT16 seq);

     // get bitmap ack count
     // negative: special code or sequence number to restart at
     // reading resets count to 0
INT16 CALLTYPE eyelink_bitmap_ack_count(void);
...
/*********** NEW CONNECTION ADDRESS ************/
    ...
    // Address used for non-connected time requests and message sends.
    // the "proper" way to do this is with the "node" type of functions
    // but we allow a "back door" to simplify higher level support functions
 
    // This is also the address used under Windows for looking for tracker
    // (an IP broadcast is used on all other platforms).
    // There is a bug in the Windows networking, causing broadcasts sent on 
    // all cards to have the IP source addres of only the first card.
    // This means the tracker sends its connection reply to the wrong address.
    // So the exact address or a subnet broadcast address (i.e. 100.1.1.255 for
    // a subnet mask of 255.255.255.0) needs to be set to that of the tracker.

void CALLTYPE eyelink_set_tracker_node(ELINKADDR node);
