...
static int comedi_init()
{
  ...
  // set or find convert gains -- some board it's software settable..
  // Try to set it for +- 10V range. <<Ben wrote: the 4th parm means
  // 'volts'. I THINK assume all channels are the same>>
  analog_range = comedi_find_range(comedi_dev,analog_in,0,0,-10,10);
  fprintf(stderr, "%s: analog_range = %d\n", progname, analog_range);
  ...
  if (use8255) {
    ...
    // no dio_config required for non-8255 card..
  }

  // we may want to use comedi_lock to lock the subdevices for this process
...
static int ad_in(int chan)
{
  ...
  if (dummymode) {
    ...
    success = comedi_data_read(comedi_dev,analog_in,
			       chan,analog_range,AREF_GROUND,&sample);
