...
file called 'INDEX' that lists each individual frame image file, **in
order**.
...
class Movie:
	def __init__(self, fb, idir, bg, x=0, y=0, smooth1=0, smooth2=0):
		self.frame = []
		self.seq = []
		self.bg = None
		...
		try:
			...
			for l in lines:
				...
				try:
					...
					warn('Warning', 'bad movie frame: <%s>' % fname, wait=1)
					sys.stderr.write('x')
					sys.stderr.flush()
			...
			sys.stderr.write('warning: no INDEX file -- using glob\n');
			lines = []
...
def _readindex(fb, x, y, idir):
	try:
		f = open(idir+'/INDEX')
		lines = f.readlines()
		f.close()
	except IOError:
		sys.stderr.write('warning: no INDEX file -- using glob *.p?m\n');
		lines = glob.glob(idir+'/*.p?m')
		if len(lines) == 0:
			warn('Warning', 'empty movie dir: %s' % idir, wait=1)
			return (None, None)
	...
	sys.stderr.write('\n')
	if nbad > 0:
		warn('Warning', '%d bad movie frames' % nbad, wait=1)
	return (framelist, seq, lines)

class Movie2:
	def __init__(self, fb, x, y, idir, bg, smooth1=None, smooth2=None):
		# keep framelist around to stop garbage collection of imagedata...
		(framelist, self.seq, self.index) = _readindex(fb, x, y, idir)
