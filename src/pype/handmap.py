...
class _Probe:
	...
	def clear(self):
		if self.major_ax:
			self.app.udpy._canvas.delete(self.major_ax)
			self.major_ax = None
		if self.minor_ax:
	...
	def draw(self):
		...
		if self.major_ax:
			self.app.udpy._canvas.coords(self.major_ax, x1, y1, x2, y2)
...
def _key_handler(app, c, ev):
	...
	if c == 'z':
		...
		app.udpy._canvas.delete(p.major_ax); p.major_ax = None
		app.udpy._canvas.delete(p.minor_ax); p.minor_ax = None
		p.force_redraw()
	elif c == 'bracketleft':
