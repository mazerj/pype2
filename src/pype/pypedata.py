...
class PypeRecord:
	...
	def __init__(self, file, recnum, rec, taskname=None,
		...
		#  rec[3]		time VECTOR (numpy/numeric array)
		#  rec[4]		eye x-pos VECTOR (numpy/numeric array)
		#  rec[5]		eye y-pos VECTOR (numpy/numeric array)
	...
	def compute(self, velocity=None, gaps=None, raw=None, nooffset=None):
		...
		if not self.computed:
			...
			self.eyet = _N.array(self.rec[3], 'f') # ms
			dt = diff(self.eyet)
			if sum(_N.where(_N.less(dt, 0), 1, 0)) > 0:
				raise PypedataTimeError
			self.realt = _N.array(self.rec[3], 'f') # ms
			self.eyex = _N.array(self.rec[4], 'f') # dva
			self.eyey = _N.array(self.rec[5], 'f') # dva
			if lag > 0:
			...
			self.eyevalid = None
			self.photo_times = _N.array(self.rec[6], 'i')
			self.spike_times = _N.array(self.rec[7], 'i')
			...
			if len(self.rec) > 13 and self.rec[13] is not None:
				...
				self.plex_times = _N.array(times, 'i')
				self.plex_channels = _N.array(channels, 'i')
				self.plex_units = _N.array(units, 'i')
				self.plex_ids = ids[:]
			...
			if gaps:
				gaps = _N.nonzero(_N.where(_N.greater(dt, 1), 1, 0))
				self.gaps_t = _N.take(self.eyet, gaps)
				self.gaps_y = self.gaps_t * 0
				self.gapdurs = _N.take(self.eyet, gaps+1) - self.gaps_t
			else:
				self.gaps_t = None
				self.gaps_y = None
...
class PypeFile:
	...
	def tdtpull(self, rec):
		...
		if self.tdt is None:
			...
			times = around(t).astype(_N.int)
			channels = around(c).astype(_N.int)
			units = around(s).astype(_N.int)
			ids = sigs[:]
...
def findfix(d, thresh=2, dur=50, anneal=10, start=None, stop=None):
	...
	for i in range(0, len(fix_ix), 2):
		a = fix_ix[i]
		b = fix_ix[i+1]
		xp = _N.sum(d.eyex[a:b]) / float(len(d.eyex[a:b]))
		yp = _N.sum(d.eyey[a:b]) / float(len(d.eyey[a:b]))
...
def fatal_unpickle_error():
		...
		sys.stderr.write("""
**************************************************
       MISSING MODULE:%s DURING UNPICKLING
**************************************************

Find the original module that is and add it to
PYTHONPATH to access this datafile.

Usually missing module imports Numeric; if you
can't find the original, make a one line file
with the same name in current dir containing:
