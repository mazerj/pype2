...
class graceGraph:
    ...
    def histoPlot(self, y, x_min=0, x_max=None, dy=None, edges=0, 
        ...
        edge_y = y.copy() #_N.zeros(len(y)+1)
        edge_y = _N.resize(edge_y, (len(y)+1,))
        edge_y[-1] = 0
    ...
    def _errPlot(self, X, Y, dy=None, symbols=None, styles=None, pType = 'xydy' ):
        ...
        for i in xrange(self.nSets, Y.shape[1] + self.nSets):
            ...
            if symbols is not None:
                ...
                self._send( '%s symbol size 0.4' % (me,) )
    ...
    def plot(self, X, Y=None, dy=None, symbols=None, styles=None, color=None):
        ...
        for i in range(self.nSets, Y.shape[1] + self.nSets):
            ...
            if color is None:
                colorno = (i%15)+1
            self._send( '%s line color %s' % (me, colorno ))
            if symbols is not None:
                self._send( me + 'symbol %s' % ((i%15) + 1) ) # From 1 to 15
                self._send( '%s symbol color %s' % (me, colorno ))
                self._send( '%s symbol size 0.4' % (me, ))
            if styles is not None:
               self._send( me + 'line linestyle %s' %((i%8) + 1) ) # 1 to 8
            self._flush()
...
def _test():
    ...
    p.hold(1)
    p.plot(joe, _N.log(joe), styles=1)
    p.legend(['Linear', 'Logarithmic'])
    p.xlabel('Abscissa')
    p.ylabel('Ordinate')
    sleep(2)
    p.focus(1,0)
    p.histoPlot(_N.sin(joe*3.14/49.0), 5./49.*3.14, 3.14)
    sleep(2)
    p.exit()
